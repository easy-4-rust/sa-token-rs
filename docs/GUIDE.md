...
### 1.4 Java vs Rust 入门对照

| 维度 | Java Sa-Token | Sa-Token-Rs |
|---|---|---|
| 项目类型 | Maven 多模块 / Spring Boot | Cargo workspace |
| 入口类 | `SaTokenContextInitializer` | `#[tokio::main] async fn main()` + `SaManager::set_xxx()` |
| Web 框架 | Spring MVC | axum 0.8 / actix-web 4 / salvo 0.85 |
| 鉴权方式 | 注解 + 拦截器 | proc-macro 宏 + Tower Layer |
| 异常处理 | `@ControllerAdvice` | axum `IntoResponse` 实现 |
| 配置载体 | `application.yml` | `config.toml` 或 `SaTokenConfig::default()` |

**Java Sa-Token（Spring Boot）入口：**

```java
@SpringBootApplication
public class App {
    public static void main(String[] args) {
        SpringApplication.run(App.class, args);  // 自动加载 sa-token-spring-boot-starter
    }
}
...
public class UserController {
    ...
    public String userInfo() {
        return "Hello, " + StpUtil.getLoginIdDefaultNull();
    }
}
```
...
#[tokio::main]
async fn main() -> SaResult<()> {
    // 初始化（替代 Spring Boot 自动装配）
    SaManager::set_config(Arc::new(SaTokenConfig::default()));
    SaManager::set_sa_token_dao(Arc::new(SaTokenDaoDefaultImpl::new()));
...
#[sa_check_login]
async fn user_info() -> SaResult<Json<String>> {
    let id = StpUtil::get_login_id_default_null().unwrap_or_default();
    Ok(Json(format!("Hello, {id}")))
}
```
...
public class MyStpInterface implements StpInterface {
    ...
    public List<String> getPermissionList(Object loginId, String loginType) {
        if ("10001".equals(loginId)) {
            return Arrays.asList("user:add", "user:list", "user:delete");
        }
...
// 检查权限
StpUtil.checkPermission("user:add");              // 缺权限抛 NotPermissionException
boolean has = StpUtil.hasPermission("user:add");

// 检查角色
StpUtil.checkRole("admin");
boolean hasAdmin = StpUtil.hasRole("admin");

// AND / OR 组合
StpUtil.checkPermissionAnd("user:add", "user:delete");  // 必须同时具有
StpUtil.checkPermissionOr("user:add", "user:list");     // 具有任一即可
```
...
// 获取指定账号 Session
SaSession s2 = StpUtil.getSessionByLoginId(10001);
...
System.out.println("token: " + info.getTokenValue() + ", login_id: " + info.getLoginId());
...
for (SaTerminalInfo t : terminals) {
    System.out.println("token=" + t.getTokenValue() + ", device=" + t.getDeviceType());
}
```
...
Sa-Token-Rs 的 `SaTokenConfig` 已实现 `serde::Deserialize`，可由 TOML / JSON / YAML 反序列化加载。下面给出一份完整可运行的示例。
...
```toml
# Sa-Token-Rs 全量配置示例（与 Java application.yml 等价）
[sa_token]
# —— Token 基础 ——
token_name = "satoken"           # 对应 Java: sa-token.token-name
token_prefix = ""                # 对应 Java: sa-token.token-prefix
token_style = "uuid"             # uuid | simple-uuid | random-32 | random-64 | random-128 | base64 | jwt

# —— 有效期 ——
timeout = 2592000                # 30 天，单位秒（Java: timeout）
active_timeout = -1              # -1 表示不检查活跃超时（Java: active-timeout）

# —— 多端登录 ——
is_concurrent = true             # 允许同账号多端登录（Java: is-concurrent）
is_share = true                  # 多人登录共用同一 Token（Java: is-share）
max_login_count = 12             # 同账号最大同时登录设备数（Java: max-login-count）

# —— Token 读取来源 ——
is_read_cookie = true            # Java: is-read-cookie
is_read_header = true            # Java: is-read-header
is_read_body = true              # Java: is-read-body
is_lasting_cookie = true         # 持久化 Cookie（Java: is-lasting-cookie）

# —— 输出与日志 ——
is_log = true                    # 是否打印运行日志（Java: is-log）
is_print = false                 # 是否打印 banner（Java: is-print）
is_write_header = true           # 是否把 Token 写到响应头（Java: is-write-header）
...
| Java (`application.yml`) | Rust (`config.toml`) | 说明 |
|---|---|---|
| `token-name` | `token_name` | Cookie / Header 名 |
| `token-prefix` | `token_prefix` | Token 前缀 |
| `token-style` | `token_style` | uuid / random-* / base64 / jwt |
| `timeout` | `timeout` | 过期秒数，-1 = 永不过期 |
| `active-timeout` | `active_timeout` | 活跃超时秒数，-1 = 不检查 |
| `is-concurrent` | `is_concurrent` | 多端登录 |
| `is-share` | `is_share` | 共享 Token |
| `max-login-count` | `max_login_count` | 同账号最大登录数 |
| `is-read-cookie` / `-header` / `-body` | `is_read_cookie` / `_header` / `_body` | Token 读取来源 |
| `is-lasting-cookie` | `is_lasting_cookie` | 持久化 Cookie |
| `is-log` / `is-print` | `is_log` / `is_print` | 日志输出 |
| `is-write-header` | `is_write_header` | 响应头回写 |
| `jwt.secret-key` | `jwt.secret_key` | JWT 密钥 |

**加载代码（main.rs 启动时执行一次）：**
...
fn load_config() -> SaResult<SaTokenConfig> {
    // 方式 1：从 TOML 文件加载
    let text = std::fs::read_to_string("config.toml")?;
    let cfg: SaTokenConfig = toml::from_str(&text)?;

    // 方式 2：从 JSON 字符串加载（等价）
    // let cfg: SaTokenConfig = serde_json::from_str(&json_text)?;

    // 方式 3：从环境变量加载（CI 场景）
    // let cfg = SaTokenConfig {
    //     timeout: std::env::var("SA_TIMEOUT").ok().and_then(|s| s.parse().ok()).unwrap_or(2592000),
...
async fn main() -> SaResult<()> {
    ...
    SaManager::set_config(cfg.clone());
    // 其他初始化...
    Ok(())
}
```
...
```rust
// 在 Cargo.toml 中加：serde_yaml = "0.9"
let text = std::fs::read_to_string("config.yaml")?;
let cfg: SaTokenConfig = serde_yaml::from_str(&text)?;
SaManager::set_config(Arc::new(cfg));
```

> **注意**：配置是 `Arc<SaTokenConfig>`，全局只读；如需热更新，构造新的 `Arc` 后调用 `SaManager::set_config` 即可。
