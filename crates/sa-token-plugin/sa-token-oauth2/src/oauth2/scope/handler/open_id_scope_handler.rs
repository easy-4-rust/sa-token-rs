...
/// Adds the Java-compatible `openid` field to access-token extra data.
pub struct OpenIdScopeHandler {
    loader: Arc<dyn SaOAuth2DataLoader>,
}

impl OpenIdScopeHandler {
    pub fn new(loader: Arc<dyn SaOAuth2DataLoader>) -> Self {
        Self { loader }
    }
}

impl SaOAuth2ScopeHandlerInterface for OpenIdScopeHandler {
    ...
    fn work_access_token(
        ...
        let openid = self.loader.get_openid(
            access_token.client_id.as_deref().unwrap_or_default(),
            &login_id_string(access_token.login_id.as_ref()),
        );
        access_token
            .extra_data
            .get_or_insert_default()
            .insert(SaOAuth2ExtraField::OPEN_ID.into(), Value::String(openid));
        Ok(())
    }
