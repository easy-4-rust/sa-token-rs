...
pub fn sa_check_login(_args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let _fn_name = &fn_item.sig.ident;
    ...
    let expanded = quote! {
        ...
        #fn_vis #fn_sig {
            // 登录检查
            if let Err(e) = ::sa_token::StpUtil::check_login() {
                return Err(e);
            }
            #fn_block
        }
...
pub fn sa_check_permission(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        #fn_vis #fn_sig {
            // 权限检查
            if let Err(e) = ::sa_token::StpUtil::check_permission(#permission_value) {
                return Err(e);
            }
            #fn_block
        }
...
pub fn sa_check_role(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        #fn_vis #fn_sig {
            // 角色检查
            if let Err(e) = ::sa_token::StpUtil::check_role(#role_value) {
                return Err(e);
            }
            #fn_block
        }
...
pub fn sa_check_safe(_args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        #fn_vis #fn_sig {
            // 二级认证检查
            if let Err(e) = ::sa_token::StpUtil::check_safe() {
                return Err(e);
            }
            #fn_block
        }
...
pub fn sa_check_disable(_args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        #fn_vis #fn_sig {
            // 封禁检查
            let login_id = ::sa_token::StpUtil::get_login_id()?;
            if ::sa_token::StpUtil::is_disable(&login_id) {
