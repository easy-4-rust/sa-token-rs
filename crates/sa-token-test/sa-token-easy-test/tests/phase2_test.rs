...
fn test_disable() {
    ...
    assert!(StpUtil::is_disable("10001").expect("disable state query"));
...
fn test_get_disable_time() {
    ...
    StpUtil::disable("10001", 60).unwrap();
    let time = StpUtil::get_disable_time("10001").expect("disable timeout query");
...
fn test_open_safe() {
    ...
    assert!(StpUtil::is_safe().expect("safe state query"));
...
fn test_switch_to() {
    ...
    assert!(StpUtil::is_switch().expect("switch state query"));
    assert_eq!(
        StpUtil::get_switch_login_id().expect("switch login id query"),
